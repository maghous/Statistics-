...
#Exemle frog
Frog = read.csv("Frog.csv", stringsAsFactors=T)
plot(Frog$SVL_mm, Frog$ED_mm, xlab="Longueur", ylab="Diamètre de l'oeil")
coeff = lm(ED_mm~SVL_mm, data=Frog)$coefficients
lines(Frog$SVL_mm, Frog$SVL_mm*coeff[2] + coeff[1], col="red")


Frog$mass_sqcb = Frog$Mass_g^(1/3)
plot(Frog$mass_sqcb, Frog$ED_mm, xlab="Masse^(1/3)", ylab="Diamètre de l'oeil")
coeff = lm(ED_mm~mass_sqcb, data=Frog)$coefficients
lines(Frog$mass_sqcb, Frog$mass_sqcb*coeff[2] + coeff[1], col="red")





modele_frog = lm(CD_mm~ED_mm, data=Frog)
prediction = predict(modele_frog, newdata=list(ED_mm=10), interval="prediction")
print(paste("Estimation du diamètre de cornée: ", prediction[1]))
